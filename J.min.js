...
var Route = {
    Name: ["DNS", "URL", "&nbsp;IP&nbsp;","RESOLV"],
    Act:  ["/json/check/dns/", "/json/check/url/", "/json/check/ip/", "/json/check/resolv/"],
    Templ:["#template-check-dns", "#template-check-url", "#template-check-ip", "#template-check-resolv"],
    Func: [
            function(str) {
            ...
            function(str) {
                var pattern = new RegExp('^((\\d{1,3}\\.){3}\\d{1,3})?$','i');
                return pattern.test(str);
            },
            function(str) {
                var pattern = new RegExp('^(https?:\\/\\/)'+
    ...
    StrError: {
        BADRFORMAT:  ", bad response format",
        BADVALIDATE: ", validate error"
    }
};
...
window.onload = function() {
    ...
    J.Ready(function () {
        ...
        AlertSetError = function (ele, errstr, rtstr, extstr) {
            J("#in-error").Html(errstr);
            J("#in-type-hdr").Html(rtstr);
            J("#in-type-err").Html(rtstr + extstr);
            ele.FadeIn();
        };

        var chkForm = J("#check-form")[0];

        if (!J.fn.isUndefined(chkForm))
        {
            chkForm.onsubmit = function (e) {

                e = e || window.event;

                var rd      = chkForm.elements["src-type"],
                    str     = chkForm.elements["in-check"].value,
                    elw     = J("#in-warning"),
                    dst     = J("#in-data");

                    dst.Html();

                if ((J.fn.isUndefined(str)) || (str == "")) {
                    AlertSetError(elw, "empty DNS, URL or IP string", "input!", "");
                    return false;
                }
                for(var i = 0; i < rd.length; i++)
                {
                    if (rd[i].checked)
                    {
                        J("#act-text").Html(Route.Name[i]);

                        if (!Route.Func[i](str)) {
                            dst.HtmlEmpty();
                            AlertSetError(elw, str, Route.Name[i], Route.StrError.BADVALIDATE);
                            break;
                        }
                        J("#in-type-hdr").Html(Route.Name[i]);
                        J.fn.GetJSON(Route.Act[i] + "?" + encodeURIComponent(str), function (data, status) {
                            if (status) {
                                elw.Hide();
                                switch (i) {
                                    case 0: {
                                        dst.Html(
                                            J(Route.Templ[i]).Template(data)
                                        );
                                        break;
                                    }
                                    case 1: {
                                    ...
                                    case 2: {
                                        var bdata   = data,
                                            idata   = null;
                                            tmplip1 = J(Route.Templ[i]),
                                            tmplip2 = null;

                                        if (!J.fn.isUndefined(bdata.result)) {
                                            idata = bdata.result;
                                            delete  bdata.result;
                                            tmplip2 = J(Route.Templ[i] + "-item");
                                        }
                                        dst.Html(
                                            tmplip1.Template(bdata)
                                        );
                                        if ((idata) && (tmplip2)) {
                                            dst.HtmlAppend(
                                                tmplip2.Template(idata)
                                            );
                                        }
                                        break;
                                    }
                                ...
                                dst.HtmlEmpty();
                                AlertSetError(elw, data, Route.Name[i], Route.StrError.BADRFORMAT);
                            }
                        });
                        break;
                    }
            ...
            J("#in-warnclose").Click(function (e) {
                J("#in-warning").FadeOut();
            });
            J("#radio-0").Click(function () {
                J("#act-text").Html(Route.Name[0]);
            });
            J("#radio-1").Click(function () {
                J("#act-text").Html(Route.Name[1]);
            });
            J("#radio-2").Click(function () {
                J("#act-text").Html(Route.Name[2]);
            });
            J("#radio-3").Click(function () {
                J("#act-text").Html(Route.Name[3]);
            });
        }

        var tmpldns = J("#template-list-dns"),
            tmplurl = J("#template-list-url"),
            tmpldld = J("#template-list-download");

        if ((!J.fn.isUndefined(tmpldns[0])) && (!J.fn.isUndefined(tmplurl[0]))) {
            J.fn.GetJSON("/json/stat/db/", function (data, status) {
                if (status) {

                    var eledns = J("#txt-templ-dns"),
                        eleurl = J("#txt-templ-url");

                    if (!J.fn.isUndefined(eledns[0])) {
                        eledns.Html(
                            tmpldns.Template(data.dnsfilter)
                        );
                    }
                    if (!J.fn.isUndefined(eleurl[0])) {
                        eleurl.Html(
                            tmplurl.Template(data.urlfilter)
                        );
                    }
                } else {
                    AlertSetError(J("#in-warning"), data, status, ", GetJSON (stat/db)");
                }
            });
        }
        if (!J.fn.isUndefined(tmpldld[0])) {
            J.fn.GetJSON("/json/download/", function (data, status) {
                if (status) {
                    var ele  = J("#txt-templ-download");
                    if (
                        (J.fn.isUndefined(ele[0])) ||
                        (J.fn.isUndefined(data.files))) { return; }

                    for (var i = 0; i < data.files.length; i++) {
                        ele.HtmlAppend(tmpldld.Template(data.files[i]));
                    }
                } else {
                    AlertSetError(J("#in-warning"), data, status, ", GetJSON (download)");
                }
            });
        }
    });
